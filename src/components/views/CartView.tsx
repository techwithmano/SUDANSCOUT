                    ...
                    return (
                        <Card key={`${item.id}-${item.size}`} className="p-4">
                            <div className="flex flex-col sm:flex-row items-center gap-4">
                                <Image src={item.imageUrl} alt={productName || item.aiHint} width={80} height={80} className="rounded-md object-cover flex-shrink-0" />
                                ...
                                <Button variant="ghost" size="icon" className="text-destructive" onClick={() => removeFromCart(item.id, item.size)}>
